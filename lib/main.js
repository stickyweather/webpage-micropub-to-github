...
queryHandler: (q, req) => {
  if (q === 'config') {
    ...
    if (req.targetsite.syndicateTo) { 
      ...
      config['mp-syndicate-to'] = req.targetsite.syndicateTo; // ADD THIS LINE
    }
    return config;
  } else if (q === 'syndicate-to') {
    return {
      'syndicate-to': req.targetsite.syndicateTo || [],
      'mp-syndicate-to': req.targetsite.syndicateTo || [] // ADD THIS LINE
    };
  }
},
